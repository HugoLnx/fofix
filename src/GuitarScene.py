...
class GuitarScene(Scene):
  ...
  def doPick3GH2(self, num, hopo = False, pullOff = False): #MFH - so DoPick knows when a pull-off was performed
    ...
    if self.instruments[num].startPick3(self.song, pos, self.controls, hopo):
      ...
      required = self.instruments[num].getRequiredNotesMFH(self.song, pos)


      colorsPressed = 0
      colorKeys = []
      for i in range(0,5):
        colorKeys.append(self.instruments[num].keys[i])

      for k in colorKeys:
        if(self.controls.getState(k) > 0):
          colorsPressed += 1

      if(colorsPressed < len(required)):
        ApplyPenalty = False
